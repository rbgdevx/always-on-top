...
1. Bumps the trailing `## Version:` segment in `AlwaysOnTop.toc` (e.g. `1.0.2` → `1.0.3`). Prefix `1.0` is preserved — edit manually for larger releases.
2. Wipes `/Applications/World of Warcraft/_retail_/Interface/AddOns/AlwaysOnTop`.
3. Mirrors this repo into it, excluding dev-only files (`.git`, `AGENTS.md`, `CHANGELOG.md`, `LICENSE`, `.libraries/`, etc. — see the script's header for the full list).
